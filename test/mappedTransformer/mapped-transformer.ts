...
export class MappedTransformer extends Transformer {
	...
	public field = (
		...
		for (const [_, resource] of Object.entries(templateResources)) {
			if (
				...
				const logicalResourceId = ResolverResourceIDs.ResolverResourceID(
					parent.name.value,
					field.name.value,
				);
				const mappings: { to: string; from: string }[] = getDirectiveArgument(
					directive,
					'mappings',
				);

				for (const mapping of mappings) {
					const snippet: string = this.createSingleFieldVTLSnippet(
						mapping.to,
						mapping.from,
					);
	...
	private createSingleFieldVTLSnippet = (
		fieldName: string,
		...
		const statements: Expression[] = [
			set(
				ref('mappedBody'),
				methodCall(ref('util.parseJson'), ref('ctx.result.body')),
			),
			qref(`$mappedBody.put('${fieldName}', $mappedBody['${fromFieldName}'])`),
			set(
				ref('ctx.result.body'),
				methodCall(ref('util.toJson'), ref('mappedBody')),
			),
		];
	...
	private validate = (field: FieldDefinitionNode) => {
		...
		const httpDirective = directives?.filter(
			(dir) => dir.name.value === 'http',
		);
		if (!httpDirective)
			throw new InvalidDirectiveError(
				'@mapped directive must be used alongside @http directive',
			);
	};
}
