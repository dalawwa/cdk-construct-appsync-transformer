...
export class AppSyncTransformer extends Construct {
  ...
  constructor(scope: Construct, id: string, props: AppSyncTransformerProps) {
    ...
    const transformerConfiguration: SchemaTransformerProps = {
      ...
      customVtlTransformerRootDirectory:
        props.customVtlTransformerRootDirectory,
    };
    ...
    this.nestedAppsyncStack = new NestedStack(
      this,
      props.nestedStackName ?? 'appsync-nested-stack',
    );
    ...
    this.appsyncAPI = new GraphqlApi(this.nestedAppsyncStack, `${id}-api`, {
      ...
      schema: SchemaFile.fromAsset(
        path.join(transformer.outputPath, 'schema.graphql'),
      ),
    ...
    this.tableNameMap = this.createTablesAndResolvers(
      tableData,
      resolvers,
      props.tableNames,
    );
    ...
    this.publicResourceArns = this.getResourcesFromGeneratedRolePolicy(
      transformer.unauthRolePolicy,
    );
    this.privateResourceArns = this.getResourcesFromGeneratedRolePolicy(
      transformer.authRolePolicy,
    );
  ...
  private createTable(tableData: CdkTransformerTable, tableName?: string) {
    ...
    const streamSpecification =
      this.props.dynamoDbStreamConfig &&
      this.props.dynamoDbStreamConfig[modelTypeName];
    const tableProps = {
      tableName,
      ...
      sortKey:
        tableData.sortKey && tableData.sortKey.name
          ...
          : undefined,
      timeToLiveAttribute: tableData?.ttl?.enabled
        ? tableData.ttl.attributeName
        : undefined,
      ...
      for (const resource of resources) {
        ...
        if (fieldName) {
          resolvedResources.push(
            `arn:aws:appsync:${region}:${account}:apis/${this.appsyncAPI.apiId}/types/${typeName}/fields/${fieldName}`,
          );
        } else {
          resolvedResources.push(
            `arn:aws:appsync:${region}:${account}:apis/${this.appsyncAPI.apiId}/types/${typeName}/*`,
          );
        }
      }
    }
  ...
  public overrideResolver(props: OverrideResolverProps) {
    const resolver = this.nestedAppsyncStack.node.tryFindChild(
      `${props.typeName}-${props.fieldName}-resolver`,
    ) as Resolver;
    if (!resolver) {
      throw new Error(
        `Resolver with typeName '${props.typeName}' and fieldName '${props.fieldName}' not found`,
      );
    }
    ...
    if (props.requestMappingTemplateFile) {
      cfnResolver.requestMappingTemplate = fs
        .readFileSync(props.requestMappingTemplateFile)
        .toString('utf-8');
    }

    if (props.responseMappingTemplateFile) {
      cfnResolver.responseMappingTemplate = fs
        .readFileSync(props.responseMappingTemplateFile)
        .toString('utf-8');
    }
  }
