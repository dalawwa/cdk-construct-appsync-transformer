...
export class CdkTransformer extends Transformer {
		...
		for (const [resourceName, resource] of Object.entries(templateResources)) {
			if (resource.Type === 'AWS::DynamoDB::Table') {
				...
				if (resource.Properties?.DataSourceName === 'NONE') {
					this.noneDataSources[
						`${resource.Properties.TypeName}${resource.Properties.FieldName}`
					] = {
					...
					const functionDependsOn =
						(functionConfiguration.DependsOn as string) ?? '';
					const functionDataSource = templateResources[functionDependsOn];
					const functionArn =
						functionDataSource.Properties?.LambdaConfig?.LambdaFunctionArn
							?.payload[1].payload[0];
					...
					this.functionResolvers[functionName].push({
						...
						defaultRequestMappingTemplate:
							MappingTemplate.lambdaRequest().renderTemplate(),
						defaultResponseMappingTemplate:
							functionConfiguration.Properties?.ResponseMappingTemplate, // This should handle error messages
					});
				} else {
					// Should be a table/model resolver -> Maybe not true when we add in @searchable, etc
					const dataSourceName =
						resource.Properties?.DataSourceName?.payload[0];
	...
	private buildTablesFromResource(
		...
		const tableResource = ctx.template.Resources
			? ctx.template.Resources[resourceName]
			: undefined;
		// console.log("##############", {
		//   resourceName,
		//   tableResource: JSON.stringify(tableResource, null, 2),
		// });
		...
		if (lsis) {
			lsis.forEach((lsi: any) => {
				const lsiKeys = this.parseKeySchema(
					lsi.KeySchema,
		...
		if (gsis) {
			gsis.forEach((gsi: any) => {
				const gsiKeys = this.parseKeySchema(
					gsi.KeySchema,
					attributeDefinitions,
				);
				const gsiDefinition = {
