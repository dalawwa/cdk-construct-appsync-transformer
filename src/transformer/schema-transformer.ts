...
export class SchemaTransformer {
  ...
  constructor(props: SchemaTransformerProps) {
    ...
    this.customVtlTransformerRootDirectory =
      props.customVtlTransformerRootDirectory ?? process.cwd();
  ...
  public getResolvers() {
    ...
    const resolversDirPath = path.normalize(
      path.join(this.outputPath, 'resolvers'),
    );
  ...
  private writeResolversToFile(resolvers: any) {
    ...
    const resolverFolderPath = path.normalize(
      path.join(this.outputPath, 'resolvers'),
    );
    ...
    Object.keys(resolvers).forEach((key: any) => {
      ...
      const resolverFilePath = path.normalize(
        path.join(resolverFolderPath, fileName),
      );
...
export class TransformerFeatureFlagProvider implements FeatureFlagProvider {
  ...
  protected getValue<T extends string | number | boolean>(
    featureName: string,
    defaultValue?: T,
  ): T {
